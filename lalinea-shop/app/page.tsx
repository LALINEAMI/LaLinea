...
<div className="mt-8 border border-yellow-400/40 bg-black/80 p-5">
  ...
  <video
    src="/products/premium-filtred/island1.MOV"
    autoPlay
    muted
    loop
    playsInline
  ...
  <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3">
  {["island2.jpg", "island3.jpg", "island4.jpg", "island5.jpg"].map(
    (file, index) => (
      <img
        key={file}
        src={`/products/premium-filtred/${file}`}
        alt={`Island Mimosa foto ${index + 1}`}
        className="w-full aspect-square object-cover"
      />
    )
  )}
    ...
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
      ...
      {[
        ...
        <button
          ...
          type="button"
          onClick={() => {
            const id = `island-mimosa-${opzione.grammi}`;
            ...
            setCarrello((prev) => {
              ...
              return [
                ...
                {
                  id: id as any,
                  nome: `ISLAND MIMOSA ${opzione.grammi}`,
                  prezzo: opzione.prezzo,
                  quantita: 1,
                },
              ];
  ...
  <p className="mt-4 text-sm font-bold uppercase text-zinc-400">
    Per quantità maggiori, contattare in privato. Nella sezione Contatti troverete tutte le info.
  </p>
